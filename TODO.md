migrate to Golang, deploy and save data, the data will be collected with streamlit python

# TODO pasarle como argumentos la palabra que quiero scrapear

# TODO poner modelo de datos en los scrapers para validar

# TODO meter busqueda de palabras clave

# ANTES DE LA PRIMERA SUBIDA

# TODO poener una carpeta desde la cual se leerán los datos

# Farmeando cookies cuando te da bien, funciona linkedin pero quizas esas cookies seran invalidas cuando te banee.

# hacer esta busqueda y si no tiene las palabras clave en la descripcion, la descarto scraping, crawling, acquisition, extraction, consuming
