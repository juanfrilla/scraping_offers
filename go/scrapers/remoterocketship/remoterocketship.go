...
func (rrs *RemoteRocketshipScraper) Name() string {
	return rrs.ScraperName
}
...
func (rrs *RemoteRocketshipScraper) parse(responseJSON *RemoteRockJobList) ([]models.ScrapedJob, error) {
	...
	for jobID, job := range jobs {
		...
		if utils.IsLocal() {
			...
			if err != nil {
				return nil, fmt.Errorf("error fetching job info: %v", err)
			}
			...
			if err != nil {
				return nil, fmt.Errorf("error parsing job HTML: %v", err)
			}
...
func (rrs *RemoteRocketshipScraper) Scrape() ([]models.ScrapedJob, error) {
	resp, err := rrs.getJobsRequest()
	if err != nil {
		return nil, fmt.Errorf("error getJobsRequest: %w", err)
	}
	defer resp.Body.Close()
	...
	if err := utils.GetJSONFromHTML(doc, &rJSON); err != nil {
		return nil, fmt.Errorf("error GettingJSONFromHTML: %w", err)
	}
